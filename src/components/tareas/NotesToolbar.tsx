...
export function NotesToolbar({ onCommand }: Props) {
  ...
  const Btn = ({ cmd, value, active: isActive, children }: { cmd: string; value?: string; active?: boolean; children: React.ReactNode }) => (
    <button
      ...
      className={`px-2 py-1.5 rounded-lg text-[14px] font-bold transition-colors whitespace-nowrap shrink-0 ${
  ...
  return (
    <div className="flex items-center justify-center bg-[#fafafa] border-t border-[#e8e8ed] px-2 py-2 overflow-x-auto no-scrollbar">
      ...
      <Btn cmd="bold" active={active.bold}><span className="font-extrabold">B</span></Btn>
      <Btn cmd="italic" active={active.italic}><span className="italic font-serif">I</span></Btn>
      <Btn cmd="underline" active={active.underline}><span className="underline">U</span></Btn>
