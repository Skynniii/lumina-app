...
export function TaskDetailView({ task, lists, onBack, onToggle, onUpdate, onDelete, onNavigate }: Props) {
  ...
  const formatDeadline = () => {
    if (!task.dueDate) return 'Fecha límite';
  ...
  const formatDate = () => {
    if (!task.scheduledDate) return 'Seleccionar fecha/hora';
        ...
        {!isCompleted && (
          <div className="border-b border-[#f0f0f5]">
            <div onClick={() => setShowDeadlinePicker(true)} className="flex items-center gap-3 py-3 cursor-pointer">
              ...
              <span className={`flex-1 text-[15px] ${task.dueDate ? 'text-[#7f70ff] font-medium' : 'text-[#555]'}`}>{formatDeadline()}</span>
              {task.dueDate && (
        ...
        {!isCompleted && (
          <div className="border-b border-[#f0f0f5]">
            <div onClick={() => setShowCalendar(true)} className="flex items-center gap-3 py-3 cursor-pointer">
              ...
              <span className={`flex-1 text-[15px] ${task.scheduledDate ? 'text-[#7f70ff] font-medium' : 'text-[#555]'}`}>{formatDate()}</span>
              {task.scheduledDate && (
