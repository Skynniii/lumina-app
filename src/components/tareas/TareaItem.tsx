    ...
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-20">
      {[...Array(6)].map((_, i) => (
        <motion.div
          ...
          animate={{ opacity: 0, scale: 1.2, x: Math.cos((i * 60 * Math.PI) / 180) * 18, y: Math.sin((i * 60 * Math.PI) / 180) * 18 }}
          transition={{ duration: 0.4, ease: 'easeOut' }}
          className="absolute w-[4px] h-[4px] bg-[#7f70ff] rounded-full"
        />
      ))}
    </div>
  );
...
export const TareaItem = memo(({ task, onToggle, onUpdate, onDeleteTask, isExpanded, isAnyExpanded, onExpand, onClose }: Props) => {
  ...
  const [sparkle, setSparkle] = useState<'checkbox' | 'button' | null>(null);
  ...
  // Scroll a la tarea cuando se expande
  useEffect(() => {
    if (!isExpanded) {
    ...
    const t = setTimeout(() => {
      ...
      const container = liRef.current.closest('.overflow-y-auto');
      if (!container) return;
      const cr = container.getBoundingClientRect();
      const lr = liRef.current.getBoundingClientRect();
      container.scrollTo({ top: container.scrollTop + (lr.top - cr.top) - 155, behavior: 'smooth' });
    }, 150);
  ...
  useEffect(() => {
    ...
    const handler = (e: PointerEvent) => {
      if (liRef.current && !liRef.current.contains(e.target as Node)) onClose();
    };
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(t);
  }, [task.text, isExpanded, editingTitle]);
  ...
  const deleteSub = (subId: string, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const completedDateText = () => {
    ...
    return d.toDateString() === new Date().toDateString() ? 'Hoy' : d.toLocaleDateString('es-CO', { day: 'numeric', month: 'short' });
  };
    ...
    <>
      <motion.li
        ...
        className={`flex flex-col transition-colors duration-300 ${
          isExpanded
            ? 'bg-white rounded-[20px] my-2 shadow-[0_12px_40px_rgba(0,0,0,0.08)] border border-[#eaeaea] z-10 relative overflow-hidden'
            : task.isImportant && !isCompleted
              ? 'bg-[#fff9e6] rounded-[16px] my-1 px-1'
              : 'bg-transparent px-1'
        }`}
      >
        {/* Fila principal */}
        <div
          onClick={() => { if (!isAnyExpanded) onExpand(task.id); }}
          className="flex items-center py-3.5 px-2 cursor-pointer w-full select-none group min-h-[50px]"
        >
          {/* Checkbox */}
          <motion.div
            animate={{ width: isExpanded ? 0 : 22, opacity: isExpanded ? 0 : 1, marginRight: 14 }}
            transition={{ duration: 0.3, ease: 'easeInOut' }}
            className="relative flex items-center justify-center h-[22px] flex-none overflow-hidden"
            onClick={(e) => handleComplete('checkbox', e)}
          >
            {sparkle === 'checkbox' && <Sparkles />}
          ...
          <textarea
            ...
            readOnly={!editingTitle || isCompleted}
            onClick={(e) => {
              if (isExpanded) {
                e.stopPropagation();
                if (!editingTitle && !isCompleted) {
            ...
            className={`bg-transparent outline-none resize-none border-none flex-grow transition-all duration-300 overflow-hidden p-0 m-0 leading-snug
              ...
              ${isCompleted && !isExpanded ? 'text-[#a0a0a0] line-through' : ''}
              ${editingTitle ? 'cursor-text' : 'cursor-pointer'}`}
        ...
        <div className={`grid transition-[grid-template-rows] duration-300 ease-[cubic-bezier(0.25,1,0.5,1)] ${isExpanded ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]'}`}>
          <div className="overflow-hidden min-h-0">
            <div className="flex flex-col px-4 pb-4 pt-2">
              ...
              {/* Fecha */}
              <div
                onClick={() => !isCompleted && setShowDatePicker(true)}
                className={`flex items-center gap-3 py-3.5 border-b border-[#f5f5f5] transition-colors -mx-4 px-4 ${!isCompleted ? 'cursor-pointer hover:bg-[#fafafc]' : ''}`}
              >
                {isCompleted ? (
                  <>
                    ...
                    <span className="text-[15px] text-[#34c759] font-medium">Completada: {completedDateText()}</span>
                  </>
                ) : (
                  <>
                    ...
                    <div className="flex-1 flex items-center justify-between">
                      <span className={`text-[15px] ${task.dueDate ? 'text-[#7f70ff] font-medium' : 'text-[#555]'}`}>{formatDate()}</span>
                      {task.dueDate && (
                        ...
                        <div className="pb-4 pl-[32px] pr-2">
                          <ul className="list-none p-0 m-0 space-y-3 mb-3">
                            {task.subtasks?.map((sub) => (
                              <li key={sub.id} onClick={() => setActiveSubId(sub.id)} className="flex items-center group relative cursor-pointer">
                                <div onClick={(e) => toggleSub(sub.id, e)} className="relative flex items-center justify-center w-[16px] h-[16px] flex-none mr-3">
                                ...
                                {!isCompleted && (
                                  <button onClick={(e) => deleteSub(sub.id, e)} className={`p-1.5 ml-2 rounded-md transition-all duration-200 border-none bg-transparent cursor-pointer ${activeSubId === sub.id ? 'text-[#ff4d4d] opacity-100' : 'text-[#ccc] opacity-0 md:group-hover:opacity-100'}`}>
                          ...
                          {!isCompleted && (
                            <div className="flex items-center mt-2">
                              ...
                              <input type="text" value={newSubtask} onChange={(e) => setNewSubtask(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addSubtask(e)} placeholder="Añadir subtarea" className="w-full bg-transparent outline-none text-[14px] text-[#444] placeholder-[#a0a0a0]" />
                            </div>
                          )}
                        </div>
                      </motion.div>
              ...
              <div className="flex justify-between items-center mt-5">
                <button onClick={(e) => { e.stopPropagation(); onDeleteTask?.(task.id); }} className="text-[#ff4d4d] p-2.5 rounded-full hover:bg-[#fff5f5] transition-colors cursor-pointer border-none bg-transparent">
