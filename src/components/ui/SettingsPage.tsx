  ...
  return (
    ...
    >
      ...
      <div className="flex-1 px-5 pb-[100px]">
        ...
        />
        <div className="space-y-3">
          <div className="p-4 bg-[#f8f8fa] rounded-2xl border border-[#eee]">
            <p className="text-[15px] font-semibold text-[#333]">Sección al abrir</p>
            <p className="text-[13px] text-[#999] mt-0.5 mb-3">Qué pestaña mostrar al iniciar la app</p>
            <div className="flex gap-2 overflow-x-auto no-scrollbar -mx-1 px-1 pb-1">
              {[
                { value: 'cronometro', label: 'Timer' },
                { value: 'habitos', label: 'Tasks' },
                { value: 'tracker', label: 'Tracker' },
                { value: 'calendar', label: 'Calendar' },
              ].map((opt) => {
                const sel = settings.defaultView === opt.value;
                return (
                  <button
                    key={opt.value}
                    onClick={() => update('defaultView', opt.value as Settings['defaultView'])}
                    className={`px-4 py-2 rounded-xl text-[13px] font-medium whitespace-nowrap transition-colors flex-none ${sel ? 'bg-[#7f70ff] text-white shadow-[2px_2px_6px_rgba(127,112,255,0.25)]' : 'bg-[#eee] text-[#888]'}`}
                  >
                    {opt.label}
                  </button>
                );
        ...
        <div className="space-y-3">
          <PillRow
            title="Nueva tarea"
            subtitle="Dónde agregar las tareas nuevas"
            options={[{ value: 'first', label: 'De primero' }, { value: 'last', label: 'De último' }]}
